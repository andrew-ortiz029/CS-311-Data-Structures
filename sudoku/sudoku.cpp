...
Hours spent: ??????
Any difficulties?: ???????
Complexity of this algorithm: O(?????)
***************************************************/
#include <iostream>
...
//***************************************************************************************
	...
	for (int r = 0; r < SIZE; r++)//for each row
		for(int c = 0; c < SIZE; c++)//go through each column
		{
			...
			{
				for (int i = 1; i <= SIZE; i++) //a number can be 1 through SIZE
	      {
		if(valid(s, r, c, i)) //call valid to see if the current number is possible
		  {
		    s[r][c] = i; //possible, then put the current number into the current slot.
