...
hashTbl::hashTbl(int s)
{
  ...
  //put NULL in each slot of the array. Remember Each slot contains a pointer to an entry.
    for (int i = 0; i < s; i++)
    {
        //table[i] = new entry(); // NULL constructor
        table[i] = NULL;
    }
}
...
void hashTbl::put(entry* e)
{
  //This function is like LL::addFront() from CS211
    ...
    string key = e->ID; // key = id from the entry
...
entry* hashTbl::get(const string& key) const
{
  ...
  */
  ...
  //look for key in the linked list. Return the pointer to the entry with key.
    ...
    string curKey = cur->getID();

    if (key == curKey) // just in case there's only one entry in the slot
    {
        return cur;
    }
    
    while (cur->next != NULL) // search LL until found
    {
        cur = cur->next;
        string curKey = cur->getID();
        if (key == curKey) // compare and return if the key is found
        {
            return cur;
        }
    }
...
entry hashTbl::remove(const string& key)
{
  ...
  */
    ...
    entry found = *cur; // static entry will follow cur and return after deletion if found

    if (key == curKey) // just in case its the first entry or theres only one entry
    {
        //now delete
        if (cur->next == NULL) // one entry
        {
            delete cur;
            table[index] = NULL; // reset index just in case
        }
        else // first entry
        {
            //now delete
            entry* del = cur->getNext();
            table[index] = del;
            delete cur; 
        }
    ...
    while (cur->next != NULL) // search LL until found
    {
        string curKey = cur->next->getID(); // must look ahead of cur to delete

        if (key == curKey) // compare, delete, and return if the key is found
        {
            // copy to static entry to return
            found = *cur->next;

            //now delete
            entry* del = cur->next->getNext();
            delete cur->next;
            cur->next = del;
            return found;
        }
        else // if the key wasnt found the iterate to the next in the LL and loop
        {
            cur = cur->next;
        }
    }

    //After traversed the list, if key wan't found, throw exception.
    throw underflow(key);
}
